...
abstract class Note implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\DB\\Map\\NoteTableMap';
    ...
    /**
     * Initializes internal state of DB\Base\Note object.
     */
     ...
     * @return $this|\DB\Note The current object (for fluent API support)
     */
    ...
    public function setSlug($v)
    {
        ...
        if ($this->slug !== $v) {
            $this->slug = $v;
            $this->modifiedColumns[NoteTableMap::COL_SLUG] = true;
        }
    ...
    public function setPost($v)
    {
        ...
        if ($this->post !== $v) {
            $this->post = $v;
            $this->modifiedColumns[NoteTableMap::COL_POST] = true;
        }
    ...
    public function setCreated($v)
    {
        ...
        if ($this->created !== null || $dt !== null) {
            if ($this->created === null || $dt === null || $dt->format("Y-m-d") !== $this->created->format("Y-m-d")) {
                ...
                $this->modifiedColumns[NoteTableMap::COL_CREATED] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : NoteTableMap::translateFieldName('Title', TableMap::TYPE_PHPNAME, $indexType)];
            $this->title = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : NoteTableMap::translateFieldName('Slug', TableMap::TYPE_PHPNAME, $indexType)];
            $this->slug = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : NoteTableMap::translateFieldName('Post', TableMap::TYPE_PHPNAME, $indexType)];
            $this->post = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : NoteTableMap::translateFieldName('Created', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\DB\\Note'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(NoteTableMap::COL_SLUG)) {
            $modifiedColumns[':p' . $index++]  = 'slug';
        }
        if ($this->isColumnModified(NoteTableMap::COL_POST)) {
            $modifiedColumns[':p' . $index++]  = 'post';
        }
        if ($this->isColumnModified(NoteTableMap::COL_CREATED)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\DB\Note The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(NoteTableMap::COL_SLUG)) {
            $criteria->add(NoteTableMap::COL_SLUG, $this->slug);
        }
        if ($this->isColumnModified(NoteTableMap::COL_POST)) {
            $criteria->add(NoteTableMap::COL_POST, $this->post);
        }
        if ($this->isColumnModified(NoteTableMap::COL_CREATED)) {
            $criteria->add(NoteTableMap::COL_CREATED, $this->created);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \DB\Note (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \DB\Note Clone of current object.
     * @throws PropelException
     */
