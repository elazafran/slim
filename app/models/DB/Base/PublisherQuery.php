...
use \Exception;
use \PDO;
use DB\Publisher as ChildPublisher;
use DB\PublisherQuery as ChildPublisherQuery;
use DB\Map\PublisherTableMap;
 ...
 *
 * @method     \DB\BookQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PublisherQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \DB\Base\PublisherQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\DB\\Publisher', $modelAlias = null)
    {
    ...
    /**
     * Filter the query by a related \DB\Book object
     *
     * @param \DB\Book|ObjectCollection $book the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByBook($book, $comparison = null)
    {
        if ($book instanceof \DB\Book) {
            ...
            throw new PropelException('filterByBook() only accepts arguments of type \DB\Book or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \DB\BookQuery A secondary query class using the current class as primary query
     */
    public function useBookQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinBook($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Book', '\DB\BookQuery');
    }
